...
if (releaseDownloads.length) {
  ...
  const weeklyDetails = document.querySelector('#weekly-downloads details');
  const weeklyToggles = document.querySelectorAll('[data-weekly-download-toggle]');

  const updateWeeklyToggles = () => {
    weeklyToggles.forEach(toggle => {
      toggle.setAttribute('aria-expanded', String(weeklyDetails?.open));
    });
  };

  weeklyDetails?.addEventListener('toggle', updateWeeklyToggles);
  updateWeeklyToggles();
  ...
  weeklyToggles.forEach(toggle => {
    toggle.addEventListener('click', () => {
      ...
      weeklyDetails.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  });
